...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.bert = BertModel.from_pretrained("/root/autodl-tmp/pretrained_weights/RadBERT/")
    ...
    def train(self, mode=True):
        ...
        if getattr(self.args, "freeze_text", False):  # 默认冻结
            # a. 关闭梯度计算
            for param in self.bert.parameters():
                param.requires_grad = False
            # b. 极其关键：强制切换到 eval 模式，彻底关闭 BERT 内部的 Dropout！
            self.bert.eval()
            # 为了避免每次迭代都打印，可以只在特定情况打印，或者不打印
            # print("❄️  [状态锁定] RadBERT 已处于绝对冻结状态 (Eval + NoGrad)")
