...
class BaseConfig(FairseqDataclass):
    data_root: str = field(
        default="/root/autodl-tmp/Data/Task07_Pancreas",
        metadata={"help": "Root directory for the decoupled medical dataset"}
    )
    # [新增] 注册 pickle 文件路径参数
    pickle_path: str = field(
        default="/root/autodl-tmp/Data/Task07_Pancreas/medical_annotations.p",
        metadata={"help": "Absolute path to the medical_annotations.p file"}
    )
    ...
    num_bins: int = field(
        default=64, metadata={"help": "number of quantization bins"}
    )
...
class BaseTask(FairseqTask):
    ...
    def setup_task(cls, cfg: DictConfig, **kwargs):
        ...
        # [新增] 注入医疗影像的多类别引导 Token
        # src_dict.add_symbol("<PANCREAS>")
        # tgt_dict.add_symbol("<PANCREAS>")
        # src_dict.add_symbol("<TUMOR>")
        # tgt_dict.add_symbol("<TUMOR>")

        # Add 2D bin tokens (PolyFormer 原版坐标 Token)[cite: 7]
        for i in range(cfg.num_bins):
            for j in range(cfg.num_bins):
                src_dict.add_symbol("<bin_{}_{}>".format(i, j))
                tgt_dict.add_symbol("<bin_{}_{}>".format(i, j))
