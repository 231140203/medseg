...
class AdjustLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, update_num, det_weight=1.0, cls_weight=1.0, reduce=True):
        ...
        # 1. 获取预测概率和真实离散标签
        lprobs, target_cls, constraint_masks, index = self.get_lprobs_and_target(model, net_output, sample)

        if constraint_masks is not None:
            constraint_masks = constraint_masks[target_cls != -1]

        # 备份一份过滤前的扁平化张量，用于计算自定义符号的惩罚
        lprobs_raw = lprobs.clone()
        target_cls_raw = target_cls.clone()

        # 过滤掉不需要计算分类损失的占位符 (通常由 pad 或坐标位产生)
        lprobs = lprobs[target_cls != -1]
        target_cls_filtered = target_cls[target_cls != -1]

        # 2. 计算基础的平滑标签分类损失 (原版逻辑)
        ...
        # ================================================================
        # 3. 【新增】：界定符 (ID=3) 的针对性加权 (Hard Anchor Penalty)
        # ================================================================
        boundary_id = 3
        # 找出当前 batch 中所有的 boundary_id 位置
        boundary_mask = (target_cls_raw == boundary_id)

        if boundary_mask.any():
            # 使用没有 label smoothing 的严格交叉熵来计算界定符的损失
            # lprobs_raw 已经是 log_softmax 的结果，直接用 NLL 即可
            raw_ce_loss = torch.nn.functional.nll_loss(
                lprobs_raw.view(-1, lprobs_raw.size(-1)),
                target_cls_raw.view(-1),
                ignore_index=-1,
                reduction='none'
            )

            # 将损失恢复为原始的 batch 形状，然后用掩码提取出 ID=3 的部分
            raw_ce_loss = raw_ce_loss.view_as(target_cls_raw)
            boundary_loss = (raw_ce_loss * boundary_mask).sum()

            # 设置一个惩罚权重（可作为超参数调优，这里先设为 5.0，即让模型对这个符号的错误敏感 5 倍）
            penalty_weight = 5.0

            # 将额外的惩罚加回到主分类损失中
            loss_cls = loss_cls + (boundary_loss * penalty_weight)

        # 归一化分类损失
        loss_cls = cls_weight * loss_cls / b

        # ================================================================
        # 4. 计算坐标回归损失 (原版逻辑，完全保留)
        # ================================================================
        token_type = sample["token_type"]
        token_type = torch.stack([token_type, token_type], -1)
        target_reg = sample["target"]  # 注意：这是浮点坐标张量，不要和分类的 target_cls 搞混

        index_reg = torch.zeros_like(target_reg).to(target_reg.device)
        index_reg[:, :2, :] = 1  # the first two tokens are bbox points

        # 严格利用 token_type == 0 过滤出坐标点进行 L1 回归计算
        target_reg = target_reg[token_type == 0]
        index_reg = index_reg[token_type == 0]

        regression_output = net_output[1].squeeze(-1)
        regression_output = regression_output[token_type == 0]

        loss_reg = F.l1_loss(target_reg[index_reg == 1], regression_output[index_reg == 1]) * det_weight
        if (index_reg == 0).any():
            loss_reg += F.l1_loss(target_reg[index_reg == 0], regression_output[index_reg == 0])
